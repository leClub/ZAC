Mission = class Mission{
	constructor( name ) {
		console.log( 'creating Misson:', name );
		this.TILE_SIZE = 500;
		this.name = name;
		this.load( name );
	}
	...
	load( name ) {
		d3.json( '/missions/' + name + '.json', function( err, json ) {
			if( err ) console.log( 'Could not load Mission:', err );
			else{
				...
				var w = this.tiles[ 0 ].length * this.TILE_SIZE,
					h = this.tiles.length * this.TILE_SIZE,
					viewBox = '0 0 ' + w + ' ' + h;
				...
				for ( var y = 0; y < this.tiles.length; y ++ ) {
					for( var x = 0; x < this.tiles[ y ].length; x ++ ) {
						var rotate = 'rotate(' + ( this.tiles[ y ][ x ].orientation * 90 ) + ','
							+ ( this.TILE_SIZE / 2 + x * this.TILE_SIZE ) + ',' 
							+ ( this.TILE_SIZE / 2 + y * this.TILE_SIZE ) + ')';

						d3.select( '#map' )
							.append( 'image' )
							.attr( 'x', x * this.TILE_SIZE )
							.attr( 'y', y * this.TILE_SIZE )
